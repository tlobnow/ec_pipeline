...
SEGMENT_TRACK_ONLY=TRUE ; EXTRACT=FALSE
...
if ([ "$SEGMENT_TRACK_ONLY" = TRUE ]); then
  LOAD_ND2=FALSE            # step 1 - read the nd2, write bf/gfp/rfp tif
  SEGMENT_CELLS=FALSE       # step 2 - Omnipose segmentation
  TRACK_CELLS=TRUE         # step 3 - btrack tracking + lineage
  INSPECT_IN_FIJI=FALSE     # step 4 - Fiji macro with track ids drawn on cells
...
# ── EXPERIMENT ──────────────────────────────────────────────────────────────
EXPERIMENT_NAME="20260910_Nicom_TIR-GFP-swap_inf_T4_MOI_0.2"
ND2_PATH="/Users/u_lobnow/Documents/etc/PhD/_DD_Manuscript/bact_microscopy/20260910_z/Nicom_TIR-GFP-swap_inf_T4_MOI0.2004.nd2"
...
INPUT_SOURCE="split"
# INPUT_SOURCE="nd2"
SPLIT_DIR="/Users/u_lobnow/Documents/etc/PhD/_DD_Manuscript/bact_microscopy/20260910_z/Nicom_TIR-GFP-swap_inf_T4_MOI0.2004_split"
...
# --- z stacks ---
# FALSE (the default) means a single-plane timelapse: everything below is
# ignored. Set TRUE for a z stack, then run the focus prep once:
#
#     python tools/split_nd2.py -c <this config> --plan-only
#
# That reads the stack, works out the sharpest plane for every frame of every
# field, and saves it as focus_plan.csv next to a QC figure. Nothing else is
# written, so it is quick to repeat and the result can be checked — or edited
# by hand if one frame chose badly.
#
# The real split then REUSES that plan instead of recomputing it:
#
#     python tools/split_nd2.py -c <this config>
#
# Segmentation, tracking and everything after read the split TIFFs and never
# see the z stack at all.
HAS_Z_STACK=TRUE
 
# Each frame is reduced to one image per channel.
#   consensus  ONE plane for the whole movie, chosen by every frame together.
#              The default. Each frame's scores are divided by that frame's
#              own mean first, so growth over time does not dominate, and
#              sixty frames decide a question no single frame can answer.
#   focus      the sharpest plane, chosen PER FRAME. Only sensible when the
#              planes are far enough apart to tell apart — the log reports
#              the margin, and on a 5-plane stack it has been under 0.5%,
#              which means the per-frame choice is following noise.
#   fixed      one plane for the whole movie, set by Z_PLANE by hand
...
# With frames 2-3 min apart the stage can move between them, so measure
# every frame rather than interpolating.
Z_FOCUS_SAMPLE_EVERY=1              # 1 = every frame; higher interpolates
 
# CHANGING Z_METHOD OR Z_PLANE MEANS RE-SPLITTING. The z choice is baked into
# the TIFFs, so an existing split is stale — split_nd2.py now notices and
# re-splits by itself, but --force always does it.
 
CROP_HALF_SIZE=200                  # 200 -> 400x400 centre crop; 0 = full frame
...
# Write segmentation_outlines.tif — a binary layer, same size as the movie,
# to drop on top of the GFP stack in Fiji. A count cannot show two cells
# segmented as one, or one filament split in two; the overlay can.
SEGMENT_WRITE_OUTLINES=TRUE
...
# --- telling divisions from segmentation splitting a cell ---
# THIS IS THE FIRST LEVER TO TRY when cells that never moved keep getting new
# ids. A division ends the mother's track and starts two new ones, so every
# false division restarts the cell — and a movie that reaches generation 15+
# in two hours is almost all false divisions.
#
# A cell split into two halves by a momentary segmentation error looks exactly
# like a division on any single frame: same area, two similar pieces. What
# separates them is time — a real division is permanent, a fragment merges
# back. So a division is only kept if both daughters stay apart this many
# frames; otherwise the mother's id goes back on both pieces.
TRACK_DIVISION_MIN_PERSIST=4
 
# --- segmentation merging two cells into one mask ---
# The opposite failure: two separate cells become one object, usually where
# the image is blurred. The tracker cannot fix that — one mask is one object
# — but it can SAY so. A mask covering this much of a previously separate
# cell counts as having absorbed it, and every such cell-frame is listed in
# 03_track/merged_cells.csv and flagged as merged_mask=1 in the GFP output.
# Those masks are not one cell, so their length and phenotype are the sum of
# several and should be excluded from per-cell measurements.
TRACK_MERGE_FRAC=0.5
 
# Coarser checks on the same question, applied per frame:
TRACK_MIN_DAUGHTER_FRAC=0.30   # the smaller piece must be this share of the pair
TRACK_MAX_DIVISION_GROWTH=1.4  # the two together must not far exceed the mother
